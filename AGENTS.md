...
- For partial CAE changes, use `python -m tests.run affected` from `app/slaves/cae`; use `--list` to review selection reasons. Run focused checks while editing and `quick` for broader integration. Run `full` only for explicitly requested full validation or release checks, after implementation is settled; do not repeat the full CPU suite after each partial edit. See `docs/development/solver-development.md` for selection ownership and reports.
