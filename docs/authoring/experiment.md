...
For tet4 deformation playback, request the active Catalog output that records a mesh-bound displacement history. The structural-analysis-modes example shows the complete task/output reference and sim.record call. A final displacement field or surface-average history cannot reconstruct nodal animation. See [Viewer deformation and playback](../manual/program/program-domain-recording.md) for scale, compatible stress overlays, and manual verification.
