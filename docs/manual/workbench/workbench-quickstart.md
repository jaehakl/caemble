...
Experiment 탭은 왼쪽 Viewer, 오른쪽 코드 편집기로 나뉘며 기본 너비는 50:50입니다. 가운데 구분선을 드래그하거나 포커스 후 방향키로 크기를 조절합니다. 하단 Console은 계속 사용할 수 있습니다. 다른 탭 레이아웃은 유지됩니다.
...
좌우 기본 비율은 50:50입니다. 왼쪽 위에는 Vars PCA, 아래에는 Vars 편집기가 표시됩니다. 오른쪽 영역의 맨 위에는 공통 가로 툴바가, 비교 영역 왼쪽에는 두 Viewer가 공유하는 데이터별 세로 툴바가 있고, 그 아래 왼쪽에는 구조와 Forward 미리보기, 오른쪽에는 실제 결과가 50:50으로 표시됩니다. 각 구분선은 드래그와 방향키로 조절합니다.
...
**Forward 모델 생성**과 **모델 업데이트**로 수동 학습할 수 있습니다. 생성한 모델이 있으면 Vars 변경 후 예측이 자동으로 갱신합니다. **선택 Run**과 **전체 후보 Run**에서는 기존 실제 데이터로 자동 학습하고 각 후보의 CAD와 예측이 준비되면 미리보기를 전환한 뒤 실행합니다. 실제 결과는 저장·다운로드가 끝나는 즉시 실제 Viewer에 표시하며 후속 Calculation 완료를 기다리지 않습니다.

새 실제 결과가 추가되면 Forward 재학습과 다음 후보의 CAD 준비를 병행하고, 갱신된 모델로 다음 예측을 생성합니다. 준비 중에는 이전 미리보기와 실제 결과를 계속 표시합니다. 다음 CAD와 예측이 준비되면 미리보기만 먼저 전환하고 실제 결과는 해당 계산이 끝나면 전환합니다. 각 Viewer의 후보·Measurement 표시로 대상을 확인할 수 있습니다. 비교용 예측은 해당 후보 실행 전의 값을 유지합니다.

학습 데이터가 없거나 학습·예측이 실패하면 CAD와 사유를 표시하고 실제 실행을 계속합니다. 새 결과가 생기면 다시 학습합니다. 취소하면 마지막 화면을 유지합니다. 배치 밖에서 새 데이터가 생기면 수동 업데이트 필요 상태를 표시합니다. 모델은 탭 이동 동안 유지하고 새로고침·Experiment/source/schema/사용자 변경 또는 Worker 소실 시 다시 생성해야 합니다.
...
각 Viewer는 최초 구조나 결과가 준비되고 화면에 표시되면 콘텐츠 전체가 보이도록 카메라를 자동으로 맞춥니다. 이후 데이터 갱신에는 카메라를 유지합니다. **전체 맞춤**을 누르면 현재 콘텐츠의 중심과 크기에 다시 맞춥니다.
