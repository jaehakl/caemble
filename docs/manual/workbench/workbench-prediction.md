...
Viewer는 항상 Geometry를 표시하므로 최신 후보의 형상도 준비합니다. 같은 입력의 준비 결과는 재사용합니다. 불투명도는 90% 또는 50%로 조절하며, Output과 차트는 상하 영역에서 독립적으로 설정합니다. **Save & Run**에서는 실행에 필요한 전체 평가·재료 검증·빌드를 수행합니다.
