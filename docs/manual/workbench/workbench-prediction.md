...
비교에는 dtype, shape와 axes가 정확히 같아야 하며, 호환되는 각 Calculation에 대해 MAE, RMSE와 최대 절대 오차를 오른쪽 카드와 **Model Details**에 표시합니다. Inverse 세부 정보에는 Target↔Re-predicted, Target↔Actual, Re-predicted↔Actual의 pairwise metric이 모두 표시됩니다. source 또는 snapshot이 현재 기준과 다르면 해당 시리즈를 겹치지 않고 Incompatible로 표시합니다. Simulation·저장·계산 실패나 Cancel은 검증 실패로 표시되고, 실제 CalculationData가 없는 대상은 개별 오류가 됩니다. 성공하면 새 Measurement를 포함하도록 모델을 자동 갱신하고 현재 방향을 다시 예측합니다. 갱신된 reference fingerprint가 검증 snapshot과 다르면 기존 Actual overlay만 숨기고 검증 기록은 Model Details에 유지합니다.
