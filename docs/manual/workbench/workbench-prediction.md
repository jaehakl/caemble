...
Calculation과 Prediction의 Sample & Run은 같은 Client build·S3 업로드·Batch 실행·후처리 경로를 사용합니다. Calculation에서는 Monte Carlo(random), Prediction에서는 위의 LHS 후보 풀과 최대 거리 선택 방식을 사용하며 별도 알고리즘 선택 창은 없습니다. 큰 BuiltMeasurement와 결과 본문은 S3로 직접 전달됩니다.
