...
Admin은 Demo의 Experiment source, Measurement, RecordedData, Calculation과 CalculationData를 Workbench에서 관리할 수 있습니다. 파생 데이터가 있는 기존 Experiment Version의 source는 잠겨 있으므로 덮어쓰지 말고 **New Version**으로 저장해야 합니다. 새 Version은 자동으로 Demo가 되지 않으며 Demo Curation에서 명시적으로 교체해야 합니다. Measurement나 Calculation을 삭제해 준비 조건이 깨져도 Demo는 자동 공개 해제되지 않고 **Not Ready**로 표시됩니다.
