...
`varsSchema`의 각 항목에는 `min`, `max`와 필요한 경우 `shape`를 작성합니다. 숫자 하나인 스칼라는 `shape`를 생략할 수 있으며 기본값은 `[]`입니다. 명시적인 `shape: []`도 유효합니다. 배열 형태인 텐서는 `[3]`, `[4, 4, 1]`처럼 차원을 반드시 지정하며, 각 차원은 JavaScript가 안전하게 표현할 수 있는 양의 정수여야 합니다.
