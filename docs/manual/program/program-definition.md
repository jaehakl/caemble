...
`experiment({...})`의 `recordedData`는 `결과이름: { task, output }`으로 선언합니다. `output`은 해당 Task가 요청한 output의 `key`입니다. 공통 빌드가 Catalog 계약에서 데이터 schema와 시각화 의미를 가져와 고정합니다. 수동 tensor/group schema는 허용하지 않습니다.
...
[Electro-Thermal Notched Bar의 canonical experiment.tsx 열기](/?help=examples&item=caemble:experiment/caemble/verified/electro-thermal-notched-bar@4.0.0)
