...
처음 입자 해석을 접한다면 **예제를 선택하고 → 설정 위치를 확인하고 → 시간에 따른 결과를 살펴보는 순서**로 읽어 보세요. [형상과 재료](program-materials.md), [Task 작성](program-task.md)을 알고 있으면 아래 설정을 따라가기 쉽습니다.

## 실행할 예제 선택

[공식 예제](/doc?help=examples)에서 다음 이름으로 검색하고 전체 소스를
...
**물리량**에서 재료(Material) 또는 저장된 속성을 선택합니다. 벡터·텐서는
...
먼저 같은 저장 시각과 물리량 성분을 선택하고, 단위와 색상 범위를 확인하세요. 입자의 움직임은 자동 시각화에서, 수치의 집계와 후처리는 기록한 Box Grid에서 살펴봅니다. 점 크기를 바꾼 화면만으로 입자 수나 해상도가 달라졌다고 판단하지 않도록 주의해 주세요.

수치 결과를 가공하려면 [Calculation 안내](../workbench/workbench-calculation.md)를, 실행에 실패했다면 [실행·결과 문제 해결](../troubleshooting/troubleshooting-runtime-results.md)을 참고하세요.
