...
정적·고유진동·주파수응답·선형 좌굴·시간해석의 지원 재료 및 비선형 조합은 현재 Solver 설명을 확인하세요. 작은 변형률 탄성의 큰 회전에는 corotational 체적 경로를 사용합니다. Neo-Hookean 고체의 정적 큰 변형에는 별도의 유한변형 FEM 경로가 있으며, 거의 비압축성 재료에는 명시적으로 MINI 혼합식을 선택할 수 있습니다. 유한변형률 소성이나 자동 보·쉘 축소를 뜻하지 않습니다. 지원 조합과 제한은 [큰 변형 solid 작성 안내](../../authoring/experiment.md#큰-변형-solid의-최종-평형)를 참고하고, 얇은 굽힘과 응력 집중에서는 공간·시간 해상도 수렴을 확인하세요.
