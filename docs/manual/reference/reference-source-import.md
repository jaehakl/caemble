...
카탈로그 예제에 등록된 Calculation은 개인 Experiment로 저장할 때 함께 저장됩니다. 기존 Experiment의 **Save As**와 **Save New Version**도 선택한 원본 Version에 저장된 Calculation 전체의 이름·설명·코드를 복사합니다. 새 Calculation은 새 ID와 revision 1을 가지며, Measurement·실행 결과·기존 검증 계약은 복사하지 않습니다. 대상 Experiment에서 Measurement를 실행하고 Calculation을 다시 검증한 뒤 저장하세요. 편집 중인 미저장 Calculation 초안은 복사 대상이 아닙니다.
