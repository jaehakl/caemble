...
class CatalogV3Tests(unittest.TestCase):
    ...
    def test_spectrometer_has_current_grating_solver_and_complete_bundle(self) -> None:
        with open_catalog() as catalog:
            example = catalog.experiment('caemble:experiment/caemble/verified/czerny-turner-spectrometer@7.0.0')
            solver = catalog.get_solver_manifest('ray-tracing', '4.0.0')
        self.assertEqual(example['title'], 'Czerny–Turner Spectrometer')
        self.assertEqual([(s['name'], s['version']) for s in example['relatedSolvers']], [('ray-tracing', '4.0.0')])
        ...
        with open_catalog() as catalog:
            ...
            for example in catalog.list_experiments(limit=100)[0]:
                expected_version = "1.0.0" if example["key"] == "hyperelastic-uniaxial" else "7.1.2" if example["repository"] == "fea" else {
                    "layered-cutaways": "6.0.0", "random-sphere-hcp-array": "6.0.0", "folded-ray-tracing": "7.0.0", "czerny-turner-spectrometer": "7.0.0",
                    "continuous-ray-optics": "1.0.0",
                    ...
                    "fdtd-drude-slab": "6.0.0", "gold-fcc-fresnel": "7.0.0", "structural-optical-results": "6.0.0",
