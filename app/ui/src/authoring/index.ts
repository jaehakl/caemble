...
const references: readonly AuthoringReference[] = [
  ...
  },
  {
    ...
    content:
      'The program defines one undecorated async def simulate(*, sim, tasks, vars). The existing CAE validator parses Python and enforces its AST/builtin/member allowlists and fixed Task/RecordedData names. Run validate_and_load_simulate in the configured CAE Python environment with the actual task_names and recorded_names. Do not import the module as unrestricted Python to check it. A validated program has not yet run its body. sim.run invokes a registered Task; state connects the calculation lineage, typed inputs carry artifact values, RecordedData accepts only resultName: { task, output }, where output is the requested key. The build freezes the Catalog data schema, semantic visualization, Task/output identity, Solver version and Catalog revision. sim.record requires a live artifact from that exact Task/output; manual schemas, arbitrary values and same-shape foreign outputs are rejected. sim.release releases runtime handles. Inspect program.py and simulation.py for accepted calls and tests for valid/invalid syntax. The Node compiler does not validate Python syntax. Full orchestration, artifact contracts, output schemas and numerical results need CAE execution.',
    sourcePaths: [
