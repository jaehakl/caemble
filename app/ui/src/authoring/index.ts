...
const references: readonly AuthoringReference[] = [
  {
    ...
    content: `The following starter files demonstrate authoring syntax. The full starter bundle contains a placeholder Task and is not a complete runnable simulation. Select a tested live Catalog Example for simulation work. Numerical outputs require one Experiment or Task Box target and gridShape. Task geometry is a separate scene; Solver-specific physical roles determine interactions. Native coupling belongs in config.exports, and automatic mesh/ray visualizations need no output declaration.\n\n${cad.core}`,
    sourcePaths: [
  ...
  },
  {
    ...
    content:
      'The program defines one undecorated async def simulate(*, sim, tasks, vars). The existing CAE validator parses Python and enforces its AST/builtin/member allowlists and fixed Task/RecordedData names. Run validate_and_load_simulate in the configured CAE Python environment with the actual task_names and recorded_names. Do not import the module as unrestricted Python to check it. A validated program has not yet run its body. sim.run invokes a registered Task; state connects the calculation lineage, typed inputs carry artifact values, RecordedData accepts only resultName: { task, output }, where output is a requested Box Grid key. Outputs require exactly one Box target and gridShape, and always preserve x/y/z/time/frequency/amplitudePhase/component axes. The build freezes the Catalog schema, semantic visualization, Task/output identity, Solver version and Catalog revision. sim.record requires a live Box Grid artifact from that exact Task/output. Native coupling artifacts come from config.exports and cannot be recorded. Mesh/ray visualizations are automatic and retain the latest successful invocation snapshot per Task separately from RecordedData. sim.release releases runtime handles. Inspect program.py and simulation.py for accepted calls and tests for valid/invalid syntax. The Node compiler does not validate Python syntax. Full orchestration, artifact contracts, output schemas and numerical results need CAE execution.',
    sourcePaths: [
  ...
  },
  {
    ...
    content:
      'CalculationInput is a read-only record-name map of complete Box Grid tensors. Each leaf contains float32/float64 dtype, a seven-dimensional shape, row-major flat data, complete x/y/z/time/frequency/amplitudePhase/component axes, boxGrid profile and runtime geometry, required tensorOrder and optional quantityKind/unit. Components are the seventh axis; do not append dimensions from tensorOrder. Real data has one value channel; complex data has amplitude and phase channels with phase in radians. Use the shared RecordedData normalization to resolve metadata. Execute only the input map. Keep schema version, Experiment/Measurement/RecordedData identities, source hash, Catalog revision, input content hash and capture time outside that map. Verify the envelope and hashes before replay. Samples and truncated rows cannot be marked complete. Normalized offline input needs no live Catalog lookup; fetching/normalizing a new remote snapshot does. An offline fixture cannot stand in for a server save preflight. Render the normalized output with the same source/input hashes; do not attach a newer source or Catalog silently to an older result.',
    sourcePaths: [
