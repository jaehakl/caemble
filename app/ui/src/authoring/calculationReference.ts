...
export const calculationAuthoringReference = {
  ...
  contract: {
    ...
    projection:
      'The frozen global boxGrid.project(leaf, { axes, representation?, component?, reduce?, frame? }) projects channels/components before canonical x/y/z/time/frequency reductions. axes preserve their order; omitted axes default to mean. Reduction methods: sum, mean, min, max, median, std (population), index (zero-based). representation: amplitude or phase; phase requires a component index. component: numeric index, magnitude (|V|), magnitudeSquared (|V|²), or { tensor: [first, second] } for a labelled 3x3 or symmetric 6-component tensor. Each tensor direction is x, y, z, or all; a concrete pair selects Tij, one all selects a row/column norm, and two all select the Frobenius norm. frame.timeSeconds synthesizes A*cos(phase+2*pi*f*t) with frequency converted to Hz; frequency sum/mean runs component-wise before nonlinear magnitude and remaining axis reductions. mean divides by the frequency sample count. DC stays constant. No extra spectral weighting or FFT normalization is applied. frame.phase is the legacy common phase in radians and cannot be combined with timeSeconds; frame.axis/time-or-frequency and frame.index freeze a sweep. Returns { dtype: float64, data, axes }. Reduce to rank 0/1/2/3 before returning.',
