...
export async function dataCommand(group: string, command: string, context: CommandContext) {
  ...
  if (command === 'slice' && result && typeof result === 'object' && 'downloadRequired' in result) {
    ...
    result = {
      ...
      dtype: stored.schema.dtype,
      dataSchema: stored.schema,
      shape: accessor.shape,
      ...(stored.data.axes === undefined ? {} : { axes: stored.data.axes }),
      ...(stored.data.boxGrid === undefined ? {} : { boxGrid: stored.data.boxGrid }),
      ...(stored.data.metadata === undefined ? {} : { metadata: stored.data.metadata }),
      ...(stored.data.provenance === undefined ? {} : { resultProvenance: stored.data.provenance }),
