...
describe('Experiment source bundle v4', () => {
  it('always adds geometry.tsx and an empty snapshot v2', () => {
    const bundle = createExperimentSourceBundle(files)
    expect(bundle.formatVersion).toBe(4)
    expect(bundle.files['geometry.tsx']).toBe('export {}\n')
    expect(bundle.geometrySnapshot).toEqual({ schemaVersion: 2, entryImports: [], modules: [] })
    expect(Object.keys(bundle.files)).toEqual(['experiment.tsx', 'geometry.tsx', 'simulate.py', 'tasks/main.tsx'])
  })
