...
export const CAD_SOURCE_API_VERSION = 10 as const
...
export type ExperimentSourceDocument = Readonly<{
  ...
  apiVersion: 7 | 8 | 9 | typeof CAD_SOURCE_API_VERSION
...
export function assertCadSourceDocument(value: unknown): asserts value is ExperimentSourceDocument {
  ...
  if (
    ...
    (document.apiVersion !== 7 &&
      document.apiVersion !== 8 &&
      document.apiVersion !== 9 &&
      document.apiVersion !== CAD_SOURCE_API_VERSION)
  ) {
    throw new CadModelError('Only Experiment source format version 2 and API version 7, 8, 9, or 10 are supported.')
  }
