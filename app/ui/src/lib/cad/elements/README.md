...
각 CAD 태그는 역할에 따라 `primitives` 또는 `operations` 아래의 자체 디렉터리에서 공개 형태와 실행 코드를 분리합니다. `definition.ts`는 props 타입과 완전한 authoring manifest를 내보내며 JSCAD를 import하지 않습니다. `runtime.ts`는 입력 검증과 Geometry 생성을 담당합니다. element 고유 props만 manifest에 기록하고 공통 `id`, `position`, `rotation`, `scale` 계약은 `authoringContract.ts` 한 곳에서 관리합니다.
...
4. `elements/manifest.json`에 definition/runtime export를 등록합니다.
