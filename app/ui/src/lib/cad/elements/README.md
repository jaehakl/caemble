...
각 CAD 태그는 역할에 따라 `primitives` 또는 `operations` 아래의 자체 디렉터리에서 공개 형태와 실행 코드를 분리합니다. `definition.ts`는 props 타입과 완전한 authoring manifest를 내보내며 JSCAD를 import하지 않습니다. `runtime.ts`는 입력 검증과 Geometry 생성을 담당합니다. element 고유 props만 manifest에 기록하고 공통 `id`, `position`, `rotation`, `scale` 계약은 `authoringContract.ts` 한 곳에서 관리합니다. Primitive의 `authoringName`은 PascalCase이고 operation은 registry `tag`와 같은 lowercase 이름을 사용합니다. 전용 transform wrapper처럼 공통 transform을 받지 않으면 `standardTransforms: false`로 선언합니다.
...
6. `npm run generate:cad-api`를 실행해 registry, JSX 선언, declaration fingerprint를 갱신합니다. 생성기는 element 고유 prop과 manifest property 문서의 일치를 검사합니다.
7. element 디렉터리에 정상 입력, 경계값, surface partition, `CadModelError` 메시지를 검증하는 테스트를 추가합니다.

공통 벡터 계산, polyline 호 길이 재샘플링, Bishop frame은 `cad/geometry`를 재사용합니다. 공통 좌표 타입은 `cad/model/types.ts`에서 가져옵니다. Material 선택, 이름 중복 검사, `scale → rotation → position` 적용은 evaluation 계층의 책임이므로 element runtime에서 반복하지 않습니다.
