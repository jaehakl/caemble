...
export function buildCadAuthoringReference({
  ...
  const core = [
    ...
    '- `surfaceGroup` members use `<geometry-id>/surface/<non-negative-index>`. Give referenced primitive/Fiber leaves explicit stable IDs and use the fixed numeric slots from the Element reference. Transforms and Boolean results retain source slots; a shell root exposes slot 0 for inner and slot 1 for outer. Never emit `/surface-N`, names, URL encoding, or aliases.',
