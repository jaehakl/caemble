...
export function MaterialCatalog({
  ...
  const parameters = useInfiniteQuery(catalogMaterialParametersInfiniteQueryOptions(listQuery))
  const models = useQuery({
    ...catalogMaterialModelsQueryOptions({ q: deferredQuery, limit: 100 }),
    retry: false,
  })
  const isModel = selectedKey?.startsWith('model.') ?? false
  const parameterDetail = useQuery({
    ...catalogMaterialParameterQueryOptions(selectedKey ?? '', Boolean(selectedKey && !isModel)),
    retry: false,
  })
  const modelDetail = useQuery({
    ...catalogMaterialModelQueryOptions(selectedKey ?? '', Boolean(selectedKey && isModel)),
    retry: false,
  })
  const rows = useMemo(() => {
