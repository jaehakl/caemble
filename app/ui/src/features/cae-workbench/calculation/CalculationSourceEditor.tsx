...
import { insertCalculationSourceAtSelection } from './calculationSourceInsertion'
...
export type CalculationSourceEditorHandle = Readonly<{
  insertAtSelection: (text: string) => boolean
}>

type CalculationSourceEditorProps = Readonly<{
...
export const CalculationSourceEditor = forwardRef<CalculationSourceEditorHandle, CalculationSourceEditorProps>(
  function CalculationSourceEditor({ diagnostic, disabled = false, sourceCode, onSave, onSourceCodeChange }, ref) {
    ...
    useImperativeHandle(
      ...
      () => ({
        insertAtSelection(text: string) {
          ...
          return editor && !disabledRef.current ? insertCalculationSourceAtSelection(editor, text) : false
        },
      }),
      [],
    )
