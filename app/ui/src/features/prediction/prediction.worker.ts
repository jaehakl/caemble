...
import { parsePredictionWorkerRequest, predictionWorkerMessageRequestId } from './protocolValidation'
...
self.onmessage = (event: MessageEvent<unknown>) => {
  ...
  let request: PredictionWorkerRequest
  try {
    request = parsePredictionWorkerRequest(event.data)
  } catch {
    if (requestId !== null) {
      respond({
        type: 'error',
        ...
        message: 'Prediction Worker 요청 계약이 올바르지 않습니다.',
      })
    }
    return
  }
