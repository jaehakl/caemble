...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  manualChunk({
    ...
    content: [
      ...
      '7. **Generate & Run**은 저장된 Experiment를 서버에 batch로 등록하고 새 Candidate와 Measurement를 서버에서 준비한 뒤 CAE worker에 배분합니다. **Save & Run**은 화면에서 확인한 Vars와 Material snapshot을 그대로 고정합니다.',
      '8. 횟수를 입력하고 **Repeat Run**을 누르면 N회 시도를 한 번의 batch로 등록합니다. 기본값은 10이며, N은 성공 횟수가 아니라 전체 시도 횟수입니다. 서버가 사용 가능한 내 Launcher에 작업을 하나씩 배분합니다.',
      '9. 접수가 완료되면 브라우저를 닫아도 Candidate 준비와 CAE 계산은 계속됩니다. 작업별 진행률과 완료·실패 알림은 **Console**에 표시됩니다. 같은 작업의 진행률은 하나의 막대로 갱신됩니다. **Setting → CAE Jobs**에서 배치 목록과 오류를 확인하고 실패한 작업을 명시적으로 재시도할 수 있으며, **Batch 취소**는 아직 끝나지 않은 작업을 취소합니다.',
  ...
  manualChunk({
    ...
    collapsed: true,
    content:
      '[Electro-Thermal Notched Bar의 canonical bundle 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/electro-thermal-notched-bar@2.0.0)',
  }),
  manualChunk({
