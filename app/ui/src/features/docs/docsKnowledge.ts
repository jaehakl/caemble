...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '`ray-tracing@0.3.0`의 반사격자는 지정한 여러 회절 차수로 광선을 나눕니다. 양의 차수 방향은 월드 좌표의 홈 방향과 형상 바깥쪽 법선의 외적으로 정합니다. 효율은 입사 파워에 대한 비율이며 합은 1 이하여야 합니다. 전파할 수 없는 차수와 남은 파워는 손실로 처리하고 다른 차수로 재분배하지 않습니다.',
      '',
      ...
      '',
      '[Spectrometer 예제의 source와 Solver 계약 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/czerny-turner-spectrometer@1.0.0)',
    ].join('\n'),
  }),
  manualChunk({
    id: 'program-simulate',
