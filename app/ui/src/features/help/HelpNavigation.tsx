...
export function HelpNavigation({ kind, item }: { kind: HelpKindId; item: string | null }) {
  return (
    <nav aria-label="사용 가이드 탐색" className="space-y-5 px-4 py-5">
      <a href={helpHref('home')} aria-current={kind === 'home' ? 'page' : undefined}
      ...
      <div className="space-y-2">
        {documentationGroups.map((group, index) => (
          <details key={group.title} className="group/navigation" open={group.ids.some((id) => id === item) || (kind === 'home' && index === 0)}>
            <summary className="flex cursor-pointer list-none items-center gap-2 rounded-md px-2 py-2 text-sm font-semibold hover:text-primary [&::-webkit-details-marker]:hidden">
              <ChevronRight className="size-3.5 shrink-0 text-muted-foreground transition-transform group-open/navigation:rotate-90" aria-hidden="true" />
              {group.title}
            </summary>
            <div className="ml-3 space-y-0.5 border-l py-1 pl-2">
              {group.ids.map((id) => publicDocuments.find((page) => page.id === id)).filter((page) => page !== undefined).map((page) => (
                <a key={page.id} href={helpHref('manual', page.id)} aria-current={item === page.id ? 'page' : undefined}
      ...
      <div className="border-t pt-5">
        ...
        {documentationCatalogs.map(({ kind: catalogKind, label }) => (
          <a key={catalogKind} href={helpHref(catalogKind)} aria-current={kind === catalogKind ? 'page' : undefined}
