...
function Harness() {
  ...
  return (
    <>
      ...
      <HelpWorkspace {...help} onNavigate={(href) => navigate(href)} />
    </>
  )
}
function mount(address = '/doc?help=home') {
  return render(
...
describe('Help workspace', () => {
  ...
  it('debounces server search while keeping local results available after a server failure', async () => {
    ...
    mount()
    const input = screen.getByRole('searchbox', { name: 'Documentation 전체 검색' })
    ...
    expect(
      within(screen.getByLabelText('Documentation 본문')).getByRole('link', {
        name: /문서가 Ready가 되지 않을 때/,
      }),
    ).toBeInTheDocument()
