...
function BoxGridControls({
  ...
  const [kind, setKind] = useState<PlotKind>('heatmap')
  const [axes, setAxes] = useState<ProjectionAxis[]>(longest.slice(0, 2))
  const [representation, setRepresentation] = useState<'amplitude' | 'phase'>('amplitude')
  const [component, setComponent] = useState<number | 'magnitude' | 'arrows'>(leaf.shape[6] === 1 ? 0 : 'magnitude')
  const [reduce, setReduce] = useState<Partial<Record<ProjectionAxis, ProjectionReduction>>>({})
  const [overlay, setOverlay] = useState(false)
  const [opacity, setOpacity] = useState(0.8)
  const [bins, setBins] = useState<number | undefined>()
  const [animation, setAnimation] = useState<'off' | 'oscillation' | 'time' | 'frequency'>('off')
  const [phase, setPhase] = useState(0),
    [frameIndex, setFrameIndex] = useState(0)
  const [playing, setPlaying] = useState(false),
    [repeat, setRepeat] = useState(true),
    [speed, setSpeed] = useState(1)
  const [fixed, setFixed] = useState<readonly [number, number] | null>(null)
  ...
  const frame =
    animation === 'oscillation'
      ...
      : animation === 'time' || animation === 'frequency'
        ? { axis: animation, index: Math.min(frameIndex, leaf.shape[animation === 'time' ? 3 : 4] - 1) }
        : undefined
  ...
  }, [leaf, optionKey, rangeKey, requestKey, component, arrowsAllowed, animation])
  useEffect(() => {
    if (!playing || busy || error || completedKey !== requestKey) return
  ...
  }, [playing, busy, error, completedKey, requestKey, animation, leaf, phase, frameIndex, repeat, speed])
  ...
  const ready = !busy && completedKey === requestKey && !error && !name.startsWith('@visualizations.')
  return (
    <div className="flex h-full min-h-0 flex-col overflow-hidden bg-white [&_button]:min-h-8 [&_button]:rounded [&_button]:border [&_button]:px-3 [&_button]:text-sm [&_button]:font-medium [&_button:disabled]:opacity-40 [&_button:focus-visible]:outline-2 [&_button:focus-visible]:outline-sky-600 [&_input[type=number]]:w-20 [&_input[type=number]]:rounded [&_input[type=number]]:border [&_input[type=number]]:p-1 [&_select]:min-h-8 [&_select]:rounded [&_select]:border [&_select]:bg-white [&_select]:px-2">
      ...
      <div className="flex shrink-0 flex-wrap items-center gap-3 border-b px-3 py-2 text-sm" aria-label="재생 설정">
        ...
        {animation !== 'off' ? (
          <>
            <button aria-pressed={playing} disabled={!!error} onClick={() => setPlaying(!playing)}>
              {playing ? '일시정지' : '재생'}
            </button>
            <input
            ...
            <span>
              {animation === 'oscillation'
                ? `${phase.toFixed(0)}°`
                : `${frameIndex} · ${leaf.axes[animation === 'time' ? 3 : 4].ticks[frameIndex]} ${leaf.axes[animation === 'time' ? 3 : 4].unit}`}
            </span>
            <label>
              속도{' '}
      ...
      {error || renderError || range[0] > range[1] ? (
        <p role="alert" className="p-3 text-red-700">
          {error || renderError || '범위 최솟값은 최댓값 이하여야 합니다.'}
        </p>
      ) : null}
      ...
      {result ? <PlotProbe key={result.scalar.axes.map((axis) => axis.name).join(',')} plot={result.scalar} /> : null}
    </div>
  )
}
