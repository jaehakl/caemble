...
export function AccountWorkspace() {
  ...
  const createToken = useMutation({
    ...
    onSuccess: async (result) => {
      ...
      await queryClient.invalidateQueries({ queryKey: runtimeQueryKeys.accessKeys(auth.queryScope) })
    },
              ...
              {tokens.isLoading ? (
                ...
                tokens.data.items.map((token) => (
                  <TableRow key={token.id}>
                    <TableCell className="font-medium">{token.name || '이름 없음'}</TableCell>
                    ...
                    <TableCell className="text-right">
                      <Button
                        ...
                        onClick={() => {
                          if (window.confirm(`${token.name || '이름 없는'} token을 폐기할까요?`))
