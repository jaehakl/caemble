...
import { materialManagerRuntimeQueryOptions } from '@/features/catalog/queryOptions'
...
import {
  ...
  materialNamesQueryOptions,
  materialParametersQueryOptions,
  materialQualifiersQueryOptions,
} from './queryOptions'
...
export function MaterialDetail({
  ...
  const materialQuery = useQuery(materialDetailQueryOptions(auth.queryScope, id, validId))
  const namesQuery = useQuery(materialNamesQueryOptions(auth.queryScope, id, validId))
  const parametersQuery = useQuery(materialParametersQueryOptions(auth.queryScope, id, validId))
  const parameterNames = [...new Set((parametersQuery.data?.items ?? []).map((parameter) => parameter.name))].sort()
  const catalogQuery = useQuery(materialManagerRuntimeQueryOptions(parameterNames, parametersQuery.isSuccess))
  ...
  const qualifiersQuery = useQuery(
    materialQualifiersQueryOptions(auth.queryScope, id, parameterIds, parametersQuery.isSuccess),
  )

  const invalidate = () => invalidateMaterialQueries(queryClient, auth.queryScope)
