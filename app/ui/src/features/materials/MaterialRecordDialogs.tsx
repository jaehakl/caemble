...
import type { FloatDataDType } from '@/lib/cad/model'
import {
...
export function MaterialEditDialog({
  ...
  const { control, handleSubmit, register, reset, watch } = useForm<MaterialEditFormValues>({
    defaultValues: { inchi: '', description: '', color: '' },
  })
  useEffect(() => {
    if (open) {
      reset({
  ...
  }, [material.color, material.description, material.inchi, open, reset])
  const mutation = useMutation({
    mutationFn: (values: MaterialEditFormValues) =>
      dbTables.Material.upsertRow([
        {
          ...material,
          inchi: values.inchi.trim() || null,
      ...
      <DialogContent>
        ...
        >
          ...
          <Controller
            control={control}
            name="color"
            render={({ field }) => <MaterialColorField onChange={field.onChange} value={field.value} />}
          />
          <DialogFooter>
...
export function MaterialNameDialog({
  ...
  const queryClient = useQueryClient()
  const queryScope = usePrivateQueryScope()
  const { control, handleSubmit, register, reset, watch } = useForm<MaterialNameFormValues>({
  ...
  useEffect(() => {
    if (open) {
      reset({
        name: record?.name ?? '',
        visibility: defaultVisibility(material, user, record?.user_id),
      })
    }
  }, [material, open, record, reset, user])
  const mutation = useMutation({
    mutationFn: (values: MaterialNameFormValues) =>
      dbTables.MaterialName.upsertRow([
        {
          ...(record ?? {}),
          material_id: material.id!,
          name: values.name.trim(),
          user_id: childOwnerId(material, user, values.visibility, record?.user_id),
        },
      ]),
    onSuccess: async () => {
      await invalidateMaterialQueries(queryClient, queryScope)
          ...
          <Controller
            ...
            render={({ field }) => (
              <VisibilityField
                ...
                value={visibilityDisabled ? 'private' : field.value}
              />
            )}
          />
          <label className="grid gap-1.5">
...
type MaterialParameterFormValues = {
  name: string
  dtype: FloatDataDType
  unit: string
  relationInputUnit: string
  relationOutputUnit: string
  source: string
  version: string
  description: string
  temperature: string
...
export function MaterialParameterDialog({
  ...
  const { control, handleSubmit, register, reset, setValue, watch } = useForm<MaterialParameterFormValues>({
    defaultValues: {
      name: '',
      dtype: 'float32',
      unit: '',
  ...
  useEffect(() => {
    ...
    let dtype: FloatDataDType = 'float32'
    let unit = ''
    let relationInputUnit = ''
    let relationOutputUnit = ''
    setComponentValue('')
    setSamples([])
    ...
    if (property) {
      ...
      dtype = stored?.dtype ?? 'float32'
      unit = stored?.unit ?? config.units[0] ?? ''
      setComponentValue(
      ...
      relationInputUnit = stored?.input.unit ?? inputConfig.units[0] ?? ''
      relationOutputUnit = stored?.output.unit ?? outputConfig.units[0] ?? ''
    ...
    reset({
      ...
      temperature:
        record?.temperature === null || record?.temperature === undefined ? '' : String(record.temperature),
      pressure: record?.pressure === null || record?.pressure === undefined ? '' : String(record.pressure),
      frequency:
    ...
    setFormError('')
  }, [catalog, initialName, material, open, record, reset, user])
  ...
  const selectCatalogName = async (entry: MaterialCatalogEntry) => {
    ...
    if (property) {
      ...
      setValue('unit', '')
      setComponentValue('')
      setValue('relationInputUnit', inputConfig.units[0] ?? '')
      setValue('relationOutputUnit', outputConfig.units[0] ?? '')
      setSamples(relationSamples(model, mergedCatalog))
    }
  }

  const formValues = watch()
  const { frequency, name, relationInputUnit, relationOutputUnit, unit } = formValues
  ...
  const mutation = useMutation({
    mutationFn: async (values: MaterialParameterFormValues) => {
      if (!isMaterialCatalogKey(values.name, activeCatalog)) {
        throw new Error('카탈로그에서 Material parameter를 선택하세요.')
      }
      const normalizedFrequency = optionalNumber(values.frequency)
      ...
      if (property && propertyConfig) {
        ...
        value = createMaterialRelationValue(
          model,
          values.relationInputUnit,
          values.relationOutputUnit,
      ...
      return dbTables.MaterialParameter.upsertRow([
        {
          ...
          pressure: optionalNumber(values.pressure),
          ...
          >
                  ...
                  <label className="grid gap-1.5">
                    ...
                    <Controller
                      ...
                      render={({ field }) => (
                        <Select
                          key={`${name}-dtype`}
                          onValueChange={(value) => field.onChange(value as FloatDataDType)}
                          value={field.value}
                        >
            ...
            <div className="grid gap-4 sm:grid-cols-3">
              ...
              <label className="grid gap-1.5">
                ...
                <Input inputMode="decimal" {...register('pressure')} />
              </label>
              <label className="grid gap-1.5">
...
export function QualifierDialog({
  ...
  const queryScope = usePrivateQueryScope()
  const [catalogOpen, setCatalogOpen] = useState(false)
  const { handleSubmit, register, reset, setValue, watch } = useForm<QualifierFormValues>({
  ...
  useEffect(() => {
    if (open) {
      reset({ name: record?.name ?? '', value: record ? String(record.value) : '' })
    }
  }, [open, record, reset])
  const mutation = useMutation({
    mutationFn: (values: QualifierFormValues) => {
      if (!names.includes(values.name)) throw new Error('카탈로그에서 qualifier를 선택하세요.')
  ...
  return (
    <>
      ...
      <QualifierCatalogPickerDialog
        ...
        onSelect={(nextName) => setValue('name', nextName)}
        open={catalogOpen}
      />
    </>
  )
