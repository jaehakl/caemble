...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  manualChunk({
    ...
    content: [
      ...
      '2. Experiment 탭에서 `experiment.tsx`, `tasks/*.tsx`, `simulate.py`를 작성합니다.',
  ...
  manualChunk({
    ...
    summary: 'experiment.tsx에서 geometry, vars, Material, group과 결과 계약을 정의합니다.',
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '`errorRate`를 사용하면 새 Candidate의 frozen Material 값이 달라질 수 있습니다. 재현 가능한 기준 계산에는 `errorRate: 0`을 사용하고, 불확실성 분석에서만 의도적으로 값을 부여하세요. 저장한 Measurement에는 실제 Material parameter snapshot이 고정됩니다.',
    ].join('\n'),
  }),
  manualChunk({
  ...
  manualChunk({
    ...
    keywords: ['Experiment Program', 'experiment.tsx', 'tasks', 'simulate.py', 'orchestration', 'formatVersion 2'],
    content: [
      'Experiment Program은 공통 정의와 solver별 task, 실행 정책을 분리합니다.',
      '',
      '| 파일 | 책임 |',
      '| --- | --- |',
      '| `experiment.tsx` | 공통 lengthUnit, geometry, varsSchema, Material, group과 최종 recordedData 계약 |',
      '| `tasks/<name>.tsx` | solver identity, 선택적 Task-local scene과 `config({ vars })` |',
      '| `simulate.py` | named task 실행 순서, 분기, artifact 전달·해제·기록 |',
      '',
      '공통 형상은 Experiment layer에 두고 특정 solver에만 필요한 보조 형상은 Task layer에 둘 수 있습니다. Task 사이의 중간 결과는 vars로 다시 계산하지 않고 typed artifact로 전달합니다.',
    ].join('\n'),
  }),
  manualChunk({
  ...
  manualChunk({
    ...
    content: [
      'Experiment와 Task TSX는 `@caemble/core`만 import합니다. 상대 경로, Task 간 import, 동적 `import()`, `require()`, URL과 다른 package는 지원하지 않습니다.',
      '',
      'Experiment 정의는 `experiment({...})`, 각 Task는 `defineTask({...})`를 default export합니다. Published Geometry module은 `Geometry<Props>` 함수 component를 default export합니다. Geometry끼리는 exact coordinate로 import하고, Experiment와 Task에서는 연결된 PascalCase root alias를 전역 JSX component처럼 바로 호출합니다.',
      '',
      'Geometry Workspace 또는 Manager에서 **Experiment에서 사용**을 선택하면 exact version을 snapshot root에 연결하고 복사 가능한 예시를 표시합니다. Root alias는 component 이름이며 `id`와는 별개입니다. Alias 변경은 정적으로 확인되는 자유 identifier와 JSX tag를 함께 변경합니다.',
      '',
      '```tsx',
      '<NotchedConductor id="conductor" />',
      '```',
      '',
      'Geometry Manager의 standalone preview는 component를 `id="preview"`만 주어 호출합니다. 첫 parameter의 구조 분해에서 직접 기본값을 제공한 custom prop은 Experiment, Task와 다른 Geometry의 호출 타입에서도 optional로 추론됩니다. 기본값이 없는 prop과 `id`는 계속 필수입니다.',
      '',
  ...
  manualChunk({
    ...
    content: [
      '| Symbol | 사용 위치 | 역할 |',
      '| --- | --- | --- |',
      '| `experiment` | `experiment.tsx` | 공통 geometry, vars, Material, group과 RecordedData schema 정의 |',
      '| `defineTask` | `tasks/*.tsx` | solver identity, Task scene과 config 정의 |',
      '| `Material` | Geometry의 `materials` | canonical property와 sampling/freeze 정책 정의 |',
