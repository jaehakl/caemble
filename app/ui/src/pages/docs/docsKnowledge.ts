...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      'Source는 순수 JavaScript의 동기 `default export` 함수 하나여야 합니다. TypeScript 문법과 JSX는 허용하지 않으며, package import는 `mathjs`의 named import만 허용합니다. Editor는 허용된 Math.js 함수 signature를 안내하고 입력과 Output 계약은 실행 과정에서 검증합니다. 디버깅에는 직접 호출하는 `console.log(...)`만 사용할 수 있고 다른 console API나 alias는 허용하지 않습니다.',
      '',
      '```js',
      ...
      "작성한 source의 비-literal computed member/property(`value[key]`, computed destructuring)는 차단됩니다. 동적 배열 접근에는 `values.at(index)`, `map`, `forEach` 또는 Math.js의 `subset`/`index`를 사용하세요. 고정된 `record['path.to.leaf']`와 숫자 literal index는 허용됩니다.",
      '',
      '### 오류와 한도',
      '',
