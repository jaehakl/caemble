...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  manualChunk({
    ...
    content: [
      ...
      '8. 기존 prepared Measurement를 실행하려면 **Run Selected**를 사용합니다. Generate & Run 실행이 실패하거나 취소되어도 새 Measurement는 Prepared 상태로 남습니다.',
      '9. 성공하면 RecordedData가 원자적으로 기록되고 Measurement는 Recorded 상태가 됩니다. 결과 저장만 실패하면 세션 결과를 유지한 채 **Retry Saving Results**로 저장을 다시 시도합니다.',
      '',
  ...
  manualChunk({
    ...
    summary: 'CAD API v9 source의 기본 구조, 필수 prop 기본값, PascalCase primitive와 제어문 활용을 함께 보여줍니다.',
    keywords: [
      'CAD API v9',
      'geometry.tsx',
      'Geometry',
    ...
    content: [
      'CAD API v9 Geometry는 JSX를 반환하는 순수 함수 component입니다. `geometry.tsx`에는 재사용할 named `Geometry<Props>`를 두고, `experiment.tsx`의 `geometry` callback에서 호출합니다. 모든 custom prop은 required/optional 표기와 관계없이 구조 분해 initializer가 있어야 하므로 `<Assembly />`처럼 props 없이 호출할 수 있습니다. 숫자로 된 길이는 모두 해당 scene의 `lengthUnit`으로 해석됩니다.',
      '',
