...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  ...
  manualChunk({
    ...
    summary: 'CAD API v7 source의 기본 구조, PascalCase primitive와 제어문 활용을 함께 보여줍니다.',
    keywords: [
      'CAD API v7',
      'geometry.tsx',
      'Geometry',
    ...
    content: [
      ...
      '',
      '`for`, `Array.from`/`map`, `if`와 조건부 JSX를 사용할 수 있습니다. 반복 횟수는 유한하고 입력으로 재현 가능해야 하며, 반복되는 sibling에는 index나 domain key로 안정적인 `id`를 만드세요. 규칙적인 격자는 JS loop보다 `array` operation을 우선합니다.',
    ].join('\n'),
  }),
  manualChunk({
