...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  ...
  manualChunk({
    ...
    content: [
      '상단 **Calculation** 메뉴는 왼쪽부터 Measurements/Vars·RecordedData·Calculation 목록, 3D Viewer, Source Editor, Output Chart를 표시합니다. 첫 패널의 **Measurements / Vars** 탭은 현재 화면 세션 동안 선택을 유지합니다. 3D Viewer 아래에는 공통 AI Agent/Console 도크가 있으며 Calculation의 `console.log`와 실행 오류도 이 Console에 누적됩니다. 우측 열은 Output Chart와 Return 요약을 기본 65/35로 나눕니다. 열 경계, 왼쪽 세 영역, 하단 도크와 Chart/Return 경계는 드래그하거나 키보드 방향키로 조절하며 마지막 Workbench draft에 저장됩니다. Viewer를 확장하면 왼쪽 목록과 Viewer 열을 합쳐 full height Viewer로 사용하고, Source Editor와 Output 열은 계속 표시합니다.',
      '',
      ...
      '',
      'Calculation 리본의 **Save**를 누르면 이름과 설명을 입력하는 저장 모달이 열리고 현재 source와 함께 명시적으로 저장합니다. 새 Calculation의 `Ctrl+S`/`Cmd+S`도 이 모달을 열며, 이미 저장된 Calculation에서는 기존 이름과 설명으로 source를 즉시 저장합니다. 저장하지 않은 source도 선택 Measurement에 대해 500ms 뒤 자동 실행하지만 Output은 DB에 저장하지 않습니다. Measurement를 바꾸면 source draft는 유지되고 새 입력으로 다시 계산합니다.',
      ...
      '',
      'Agent에는 현재 source, 선택 Measurement ID, RecordedData의 path/dtype/shape/axis tick 앞뒤 표본/QuantityKind/tensorOrder/unit/유효성, 최신 preview의 상태·오류 또는 Output 표본만 전달됩니다. 전체 RecordedData 값은 자동 전송하지 않습니다. 필요한 경우 Agent가 Measurement detail에서 RecordedData ID를 확인한 뒤 bounded slice 도구로 일부 값만 읽습니다. Agent가 source를 작성해도 compile·실행 검증을 수행했다고 간주하지 않으며, 브라우저의 500ms preview와 Console을 확인한 뒤 리본 **Save**로 명시적으로 저장하세요.',
      '',
      ...
      '',
      '작성하는 Output은 `dtype`, finite real `data`, 선택적인 `axes`만 가집니다. `shape`를 작성하면 오류이며 scalar는 `[]`, flat array는 `[length]`, 직사각 2D array는 `[rows, columns]`, Math.js Matrix는 `size()`에서 자동 추론합니다. axes를 생략하면 `index` 또는 `row`/`column` ordinal ticks를 생성하고, 제공하면 모든 축과 ticks 길이가 추론 shape에 정확히 맞아야 합니다. Output에는 QuantityKind와 값 unit이 없습니다. rank 0은 scalar, rank 1은 line, rank 2는 heatmap으로 표시하며 Return 패널에는 dtype, 추론 shape와 row-major raw data 일부를 표시합니다. Complex, NaN, Infinity, ragged data와 rank 3 이상은 거부됩니다.',
  ...
  manualChunk({
    ...
    content: [
      '상단 **Prediction** 메뉴는 현재 Experiment의 저장된 Recorded Measurement를 학습 데이터로 사용합니다. 왼쪽 **Vars**와 오른쪽 **Calculation Data** 사이의 중앙 3D Viewer 및 하단 AI Agent/Console은 다른 Workbench와 같은 상태로 유지됩니다.',
      '',
      '### 마지막 편집이 예측 방향을 정합니다',
      '',
      ...
      '- 오른쪽 CalculationData를 편집하거나 기준 Measurement를 선택하면 **Inverse**가 됩니다. 선택한 CalculationData target 전체에서 vars를 kNN으로 예측하고, 완전한 vars 묶음을 한 번에 Candidate와 Viewer에 적용합니다. 입력한 target은 유지되며 Forward surrogate도 다시 계산합니다.',
      ...
      '- 각 CalculationData 카드의 **Display range**는 새 Forward 결과나 기준 Measurement를 불러올 때 primary 실제 min/max에 5% 여백을 더해 한 번 맞춘 뒤 고정됩니다. Target 편집, Inverse, Re-predicted 또는 Actual 도착만으로는 다시 확대·축소하지 않습니다. **Fit**은 현재 primary와 ready 상태인 호환 비교 시리즈 전체로 범위를 다시 맞춥니다. 범위 밖 cell은 끝 색으로 표시하고 `clipped` 수를 알리지만 데이터 값이나 dtype 허용 범위를 변경하지 않습니다.',
      '- Target heatmap의 선택 사각형은 Target commit, Inverse 시작·완료, Re-predicted·Actual 갱신과 Fit 뒤에도 유지됩니다. 새 Forward 결과·새 기준 Measurement처럼 primary가 교체되거나 dtype, shape, axes가 바뀔 때만 선택을 초기화합니다.',
      '',
      '### Settings와 cohort',
      '',
      '리본의 **Prediction Settings**에서 한 개 이상의 저장된 Calculation, 각 Calculation의 거리 weight, k의 Auto/Manual 방식과 Neighbor weighting을 선택한 뒤 **적용**합니다. 설정 초안은 적용 전까지 현재 모델을 바꾸지 않습니다. **새로고침**은 최신 Measurement와 CalculationData를 다시 읽고, **누락 데이터 계산**은 선택한 Calculation의 미저장 조합을 계산한 뒤 데이터를 다시 불러옵니다. Inverse 거리에는 각 Calculation tensor가 원소 수와 관계없이 하나의 블록으로 동등하게 참여하며, weight로 상대적 중요도를 조절합니다. 0은 해당 블록을 무시하므로 선택한 블록 중 하나 이상은 양수여야 합니다.',
      '',
      'Cohort에는 현재 Experiment의 Recorded Measurement 중 필요한 블록이 모두 있고 숫자가 유한하며 layout이 정확히 호환되는 행만 포함됩니다. key/path, 전체 data schema, dtype, shape, axes와 ticks, unit·QuantityKind·tensorOrder 같은 metadata가 다르면 별도 cohort로 분리되고, 가장 큰 동일-layout 그룹을 사용합니다. Forward는 현재 RecordedData schema와 정확히 일치하고 모든 leaf가 numeric인 행의 vars와 RecordedData 전체 tensor를 사용합니다. Inverse는 선택한 모든 CalculationData가 저장된 Measurement만 사용하고 CalculationData 전체 tensor를 평균이나 표준편차로 요약하지 않습니다. 제외 수와 사유는 Prediction Settings의 Cohort 요약 및 **Model Details**에서 확인할 수 있습니다.',
      '',
