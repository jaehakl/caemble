...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  ...
  manualChunk({
    ...
    content: [
      ...
      '```',
      '',
      'Candidate는 선언한 shape와 정확히 같은 dense rectangular numeric tensor여야 합니다. 축별 범위가 다르면 `positionX`, `positionY`, `positionZ`처럼 의미별 scalar 변수로 분리한 뒤 Geometry callback에서 다시 조합하세요. Geometry callback은 `({ vars })`로 값을 받고 외부의 변경 가능한 상태에 의존하지 않아야 합니다. 같은 parent 아래의 component `id`는 고유해야 하며, 이 ID를 `geometryGroup`에 넣어 `experiment.geometry.<group>`으로 참조합니다. CAD API v11의 surface member는 `<geometry-id>/surface/<non-negative-index>` 형식이며 Geometry Catalog에 표시된 primitive별 고정 slot을 사용합니다.',
      '',
      '[DC Uniform Bar의 canonical experiment.tsx 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/dc-uniform-bar@3.0.0)',
    ].join('\n'),
  }),
  manualChunk({
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '[DC Uniform Bar의 canonical Task source 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/dc-uniform-bar@3.0.0)',
    ].join('\n'),
  }),
  manualChunk({
    ...
    content: [
      ...
      '### Detector와 ray path',
      '',
      'Detector surface에서 irradiance, detected power, source 대비 efficiency를 기록할 수 있습니다. Ray path 시각화가 필요하면 `@caemble/ray-paths@1/primary/` 아래의 reserved system result인 `vertices`, `path-offsets`, `segment-power`, `path-wavelength`, `segment-event` 다섯 record를 함께 선언·기록하세요. 각 path는 최대 32개 segment를 담고 `maxPaths`는 65,536 이하입니다. Workbench는 이 다섯 텐서를 하나의 **Ray paths** 결과로 그룹화하고, 기록된 모든 경로를 기본으로 3D Viewer에 표시합니다.',
      '',
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '[DC Uniform Bar의 canonical simulate.py 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/dc-uniform-bar@3.0.0)',
    ].join('\n'),
  }),
  manualChunk({
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '- [DC Uniform Bar](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/dc-uniform-bar@3.0.0)',
      '- [DC Notched Current Density](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/dc-notched-current-density@3.0.0)',
      '- [DC Resolution Study](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/dc-resolution-study@3.0.0)',
      '- [Electro-Thermal Uniform Bar](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/electro-thermal-uniform-bar@3.0.0)',
      ...
      '- [Basketball Goal](/docs?section=solvers&item=experiment:caemble:experiment/caemble/getting-started/basketball-goal@2.0.0)',
      ...
      '- [Geometry Authoring Skeleton](/docs?section=solvers&item=experiment:caemble:experiment/caemble/getting-started/geometry-authoring-skeleton@2.0.0)',
  ...
  manualChunk({
    ...
    collapsed: true,
    content:
      '[Electro-Thermal Uniform Bar의 canonical bundle과 verification 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/verified/electro-thermal-uniform-bar@3.0.0)',
  }),
  manualChunk({
  ...
  manualChunk({
    ...
    summary: 'CAD API v11 source의 기본 구조, 필수 prop 기본값, PascalCase primitive와 제어문 활용을 함께 보여줍니다.',
    keywords: [
      'CAD API v11',
      'geometry.tsx',
      'Geometry',
    ...
    content: [
      'CAD API v11 Geometry는 JSX를 반환하는 순수 함수 component입니다. `geometry.tsx`에는 재사용할 named `Geometry<Props>`를 두고, `experiment.tsx`의 `geometry` callback에서 호출합니다. 모든 custom prop은 required/optional 표기와 관계없이 구조 분해 initializer가 있어야 하므로 `<Assembly />`처럼 props 없이 호출할 수 있습니다. 숫자로 된 길이는 모두 해당 scene의 `lengthUnit`으로 해석됩니다.',
      '',
      '[AI Agent가 사용하는 canonical Geometry Authoring Skeleton Experiment 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/getting-started/geometry-authoring-skeleton@2.0.0)',
      '',
  ...
  manualChunk({
    ...
    content: [
      ...
      '- CAD API v11 surface는 `<geometry-id>/surface/<non-negative-index>`로 참조합니다. 예를 들어 `conductor.body` Box leaf의 local +X slot은 `conductor.body/surface/1`입니다. leaf에 명시적 `id`를 주고 Geometry Catalog에 표시된 고정 slot을 사용하세요.',
      '- CAD API v7-v10 bundle은 Catalog 이력으로 계속 열어 읽을 수 있지만 compile하거나 Solver를 실행할 수 없습니다. 자동 변환이나 alias를 두지 않으므로 원본 Version은 보존하고 CAD API v11의 새 Version에서 명시적으로 이행하세요.',
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '[Basketball Goal canonical Experiment bundle 열기](/docs?section=solvers&item=experiment:caemble:experiment/caemble/getting-started/basketball-goal@2.0.0)',
    ].join('\n'),
  }),
  manualChunk({
    ...
    Object.freeze({
      ...
      content: [
        ...
        ...(entry.surfaces.length
          ? entry.surfaces.map(
              (surface) => `- \`surface/${surface.index}\` — ${surface.label}: ${surface.description}`,
            )
