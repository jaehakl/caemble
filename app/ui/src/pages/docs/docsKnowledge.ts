...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '**Vars**는 현재 `varsSchema`의 key를 선언 순서대로 표시합니다. key를 열면 scalar와 1-D Tensor는 min/max 고정 축의 막대그래프로, 2-D Tensor는 min/max 고정 색상 범위의 heatmap으로 편집합니다. N-D Tensor는 마지막 두 축을 heatmap으로 사용하고 앞 축 index 조합을 8개씩 페이지로 표시합니다. 막대는 클릭·드래그 위치를 따라가며, heatmap은 사각 영역을 드래그한 뒤 휠 또는 직접 입력으로 일괄 변경합니다. 휠 한 notch는 범위의 1%, Shift는 10배, Alt는 0.1배이고 모든 값은 min/max 안으로 제한됩니다.',
      '',
