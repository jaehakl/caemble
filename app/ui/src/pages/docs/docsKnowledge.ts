...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      'Geometry 탭은 Experiment와 독립적인 Geometry Manager입니다. Official Catalog와 Workspace의 Published Version을 Viewer에서 확인하고 Local Draft로 복제해 편집할 수 있지만, 이 선택과 draft는 현재 Experiment의 `geometry.tsx`, 저장, Simulation, AI context를 바꾸지 않습니다.',
      '',
      'Published Version은 직접 덮어쓰지 않으며 **Edit as New Version**으로 만든 Local Draft를 새 exact Version으로 발행합니다. 이전 세션의 `geometry.tsx`가 `@local`을 참조하면 미리보기는 유지되지만, Experiment 저장 전에 사용자가 해당 Geometry를 발행하고 exact import로 직접 교체해야 합니다. 자동 발행이나 source 자동 수정은 하지 않습니다.',
      '',
  ...
  manualChunk({
    ...
    summary: 'CAD API v8 source의 기본 구조, 필수 prop 기본값, PascalCase primitive와 제어문 활용을 함께 보여줍니다.',
    keywords: [
      'CAD API v8',
      'geometry.tsx',
      'Geometry',
