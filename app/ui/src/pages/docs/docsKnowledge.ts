...
export const manualDocsKnowledge: readonly DocsKnowledgeChunk[] = Object.freeze([
  manualChunk({
    ...
    content: [
      ...
      '2. Experiment 탭에서 `experiment.tsx`, `geometry.tsx`, `tasks/*.tsx`, `simulate.py`를 작성합니다.',
  ...
  manualChunk({
    ...
    content: [
      ...
      '',
      '공통 형상과 Task 보조 형상은 `geometry.tsx`에서 named export하고 `experiment.tsx`와 각 Task에서 상대 import합니다. Task 사이의 중간 결과는 vars로 다시 계산하지 않고 typed artifact로 전달합니다.',
    ].join('\n'),
  }),
  manualChunk({
  ...
  manualChunk({
    ...
    content: [
      'Experiment TSX는 `@caemble/core`와 `./geometry`, Task TSX는 `@caemble/core`와 `../geometry`의 named import만 사용합니다. Task 간 import, 동적 `import()`, `require()`, URL과 다른 package는 지원하지 않습니다.',
      '',
      'Experiment 정의는 `experiment({...})`, 각 Task는 `defineTask({...})`를 default export합니다. `geometry.tsx`와 Published Geometry module은 PascalCase named `Geometry<Props>` 함수 component를 여러 개 export할 수 있습니다. Geometry dependency는 exact coordinate의 named import 문이 유일한 원본이며 Tree와 DB projection은 source에서 자동으로 만들어집니다.',
      '',
      '```tsx',
      '// geometry.tsx',
