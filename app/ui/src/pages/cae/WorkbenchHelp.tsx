...
import { catalogDocsKnowledge, manualDocsKnowledge } from '@/pages/docs/docsKnowledge'
...
export function WorkbenchHelpExplorer({
  ...
  const quantityKinds = useQuery({
    queryKey: catalogQueryKeys.quantityKinds({ q: query.trim(), limit: 100 }),
    queryFn: () => catalogApi.listQuantityKinds({ q: query.trim(), limit: 100 }),
    enabled: kind === 'quantity-kinds',
  })
  const materialParameters = useQuery({
    queryKey: catalogQueryKeys.materialParameters({ q: query.trim(), limit: 100 }),
    queryFn: () => catalogApi.listMaterialParameters({ q: query.trim(), limit: 100 }),
    enabled: kind === 'materials',
  })
  const materialModels = useQuery({
    queryKey: catalogQueryKeys.materialModels({ q: query.trim(), limit: 100 }),
    queryFn: () => catalogApi.listMaterialModels({ q: query.trim(), limit: 100 }),
    enabled: kind === 'materials',
  })
  const solvers = useQuery({
    queryKey: catalogQueryKeys.solvers({ q: query.trim(), limit: 100 }),
    queryFn: () => catalogApi.listSolvers({ q: query.trim(), limit: 100 }),
    enabled: kind === 'solvers',
  })
...
export function WorkbenchHelpDetail({ kind, selectedItem }: { kind: HelpKindId; selectedItem: string | null }) {
  const materialParameter = useQuery({
    queryKey: catalogQueryKeys.materialParameter(selectedItem ?? ''),
    queryFn: () => catalogApi.getMaterialParameter(selectedItem!),
    enabled: kind === 'materials' && Boolean(selectedItem && !selectedItem.startsWith('model.')),
  })
  const materialModel = useQuery({
    queryKey: ['catalog', 'material-model', selectedItem ?? ''],
    queryFn: () => catalogApi.getMaterialModel(selectedItem!),
    enabled: kind === 'materials' && Boolean(selectedItem?.startsWith('model.')),
  })
  const quantityKind = useQuery({
    queryKey: catalogQueryKeys.quantityKind(selectedItem ?? ''),
    queryFn: () => catalogApi.getQuantityKind(selectedItem!),
    enabled: kind === 'quantity-kinds' && Boolean(selectedItem),
  })
  ...
  const solver = useQuery({
    queryKey: catalogQueryKeys.solver(solverName, solverVersion),
    queryFn: () => catalogApi.getSolver(solverName, solverVersion),
    enabled: kind === 'solvers' && Boolean(solverName && solverVersion),
  })
