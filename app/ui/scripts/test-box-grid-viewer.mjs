...
try {
  ...
  await ready()
  const sceneResults = page.locator('[data-viewer-scene-results]')
  assert.equal((await sceneResults.innerText()).trim(), '')
  assert.ok((await sceneResults.boundingBox()).height <= 1, 'Result descriptions leave no band above the 3D canvas')
  ...
  assert.equal((await sceneResults.innerText()).trim(), '')
  assert.ok((await sceneResults.boundingBox()).height <= 1)
