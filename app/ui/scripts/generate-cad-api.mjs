...
export type InteractionModelInstance = keyof InteractionModelParameterMap extends never
  ...
  : { [Key in keyof InteractionModelParameterMap]: Readonly<{ model: Key; parameters: InteractionModelParameterMap[Key] }> }[keyof InteractionModelParameterMap]
export type MaterialInteractionOptions = Readonly<{ between: readonly [Material, Material]; models?: Readonly<Record<string, InteractionModelInstance>> }>
export class MaterialInteraction<V extends Vars = Vars> {
  constructor(name: string, options: MaterialInteractionOptions | ((context: Readonly<{ vars: Readonly<V> }>) => MaterialInteractionOptions))
