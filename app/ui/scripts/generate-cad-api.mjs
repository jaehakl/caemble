...
const opaqueQuantityKindNames = quantityModule.opaqueQuantityKindNames
if (!quantityKindData || !Array.isArray(opaqueQuantityKindNames)) {
  throw new Error('QuantityKind data or opaque-unit policy could not be loaded for declaration generation.')
}
const {
  materialModelByKey,
...
if (
  opaqueQuantityKindNames.length !== 6 ||
  new Set(opaqueQuantityKindNames).size !== opaqueQuantityKindNames.length ||
  opaqueQuantityKindNames.some((name) => !Object.prototype.hasOwnProperty.call(quantityKindData, name))
) {
  throw new Error('Opaque QuantityKind names must contain six unique catalog entries.')
}
