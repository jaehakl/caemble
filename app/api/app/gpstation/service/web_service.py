...
async def reconcile_disconnected_launchers(
    ...
    return await job_orchestrator.reconcile_disconnected_launchers(
        db,
    ...
    *,
    ...
    if search_text:
        clauses.append(
            or_(
                *[
                    cast(getattr(model, field_name), Text).ilike(
                        f"%{search_text}%"
                    )
    ...
    for field_name, bounds in body.filter.items():
        ...
        for index, comparison in ((0, "minimum"), (1, "maximum")):
                ...
                if python_type is datetime:
                    value = (
                        ...
                        if isinstance(bounds[index], datetime)
                            ...
                            str(bounds[index]).replace("Z", "+00:00")
                        )
                    )
            ...
            range_conditions.append(
                column >= value if comparison == "minimum" else column <= value
            )
        if range_conditions:
...
def serialize_row(row: Any, fields: tuple[str, ...]) -> dict[str, Any]:
    return {
        field_name: serialize_value(getattr(row, field_name, None))
