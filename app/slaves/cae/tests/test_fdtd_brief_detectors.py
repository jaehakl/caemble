...
@pytest.mark.asyncio
async def test_brief_detector_records_selected_cell_coordinates_and_boundaries(monkeypatch, scale):
    ...
    invocation = SimpleNamespace(execution_mode="brief", world={}, config={"outputs": [{
    ...
    invocation.execution_mode = "full"
    with pytest.raises(ValueError, match="does not contain an FDTD cell center on axis 2"):
        await prepare_detectors(invocation, SimpleNamespace(domain=domain))
...
def test_existing_samples_and_stride_are_unchanged():
    ...
    ticks = ((-1., 1., 3., 5., 7., 9.),) * 3
    bounds = ((0.5, 7.5),) * 3
    full = detector_indices(bounds, core, ticks, (2, 1, 3), "detector")
    brief = detector_indices(bounds, core, ticks, (2, 1, 3), "detector", nearest_core_cell=True)
    assert [axis.tolist() for axis in brief] == [[1, 3], [1, 2, 3, 4], [1, 4]]
    for expected, actual in zip(full, brief, strict=True):
