...
def resolve_value(
    ...
    if isinstance(node, StructuredGridResource):
        value = StructuredGridValue(
            node.shape,
            tuple(resolve_value(store, axis, memo) for axis in node.axes),
            node.unit,
            node.identity,
    ...
    if isinstance(node, UnstructuredMeshResource):
        value = UnstructuredMeshValue(
            resolve_value(store, node.points, memo),
            resolve_value(store, node.cells, memo),
            node.unit,
            node.identity,
            resolve_value(store, node.metadata, memo),
        )
    ...
    if isinstance(node, FieldResource):
        value = FieldValue(
            ...
            node.quantity_kind,
            node.unit,
            resolve_value(store, node.values, memo),
    ...
    if isinstance(node, ParticleSetResource):
        value = ParticleSetValue(
            resolve_value(store, node.positions, memo),
            node.unit,
    ...
    if isinstance(node, RaySetResource):
        value = RaySetValue(
            resolve_value(store, node.origins, memo),
            resolve_value(store, node.directions, memo),
            node.unit,
            resolve_value(store, node.attributes, memo),
            node.identity,
            resolve_value(store, node.metadata, memo),
    ...
    if isinstance(node, StructuredBundleResource):
        value = BundleValue(
            node.bundle_type,
            resolve_value(store, node.members, memo),
            resolve_value(store, node.metadata, memo),
        )
        memo[ref.resource_id] = value
        return value
    if isinstance(node, MappingResource):
        resolved = {
            key: resolve_value(store, child, memo)
    ...
    if isinstance(node, SequenceResource):
        value = tuple(resolve_value(store, child, memo) for child in node.items)
        memo[ref.resource_id] = value
        return value
