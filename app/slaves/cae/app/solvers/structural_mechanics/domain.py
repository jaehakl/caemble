"""기존 Task 초기화와 frozen Material을 계산 모델로 옮긴다.

명시적 node ID는 연결의 의미다. 좌표가 같다는 이유로 절점을 합치거나
CAD 외표면에서 보 중심선/쉘 두께를 추측하지 않는다. 원래 Geometry와
Material snapshot은 읽기만 하고, 수치용 배열은 Solver 내부에서 구성한다.
"""
...
from .beam import beam_frame, isotropic_beam_section, physical_beam_mass
...
from .meshing import brick_mesh, cylinder_mesh, line_mesh, plate_mesh
from .model import Element, StructuralModel
from .shells import laminate_section
...
def selected_material(invocation, rule, role):
    source, _, group = rule["target"][0].split(".", 2)
    part = geometry_part(invocation.world[source], group)
