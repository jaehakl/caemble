...
from app.methods.structured import VoxelDomain, build_voxel_domain, structured_grid_value
...
async def build_heat_domain(context: SolverInvocation) -> HeatDomain:
    ...
    source = surface(scene, target_group(boundaries[0], "surface"))
    reference = surface(scene, target_group(boundaries[1], "surface"))
