...
class LinearFlowSystem:
    ...
    def __init__(self, mesh, velocity_operators, pressure_operators, momentum_matrix, momentum_rhs,
        ...
        self.base_velocity = self.momentum_factor.solve(momentum_rhs - volume[:, None] * self.gradient_boundary)
    ...
    async def solve(self, initial_pressure=None, *, max_iterations=1000, tolerance=1e-8,
                    cancellation=None, progress=None, stage="stokes-iteration"):
        ...
        if not np.any(self.rhs):
            pressure.fill(0)
        iteration, restart = 0, min(40, max(1, len(self.free)))
        ...
        while iteration <= max_iterations:
            ...
            if iteration % restart == 0:
                ...
                if beta == 0:
                    break
                basis = np.zeros((len(self.free), restart + 1))
                basis[:, 0] = residual[self.free] / beta
                preconditioned = np.zeros((len(volume), restart))
                hessenberg = np.zeros((restart + 1, restart))
            local = iteration % restart
            ...
            target = np.zeros(local + 2)
            target[0] = beta
            coefficients = np.linalg.lstsq(hessenberg[:local + 2, :local + 1], target, rcond=None)[0]
            pressure = base_pressure + preconditioned[:, :local + 1] @ coefficients
            iteration += 1
        face = int(np.argmax(mesh.nonorthogonality))
