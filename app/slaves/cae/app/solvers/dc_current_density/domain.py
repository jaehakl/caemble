...
from app.methods.structured import VoxelDomain, axis_ticks, build_voxel_domain, structured_grid_value, voxel_index
from app.kernel.api import SolverInvocation, StructuredGridValue
...
async def build_dc_domain(context: SolverInvocation) -> DcDomain:
    ...
    surface_source = [item for item in boundary_conditions if item["methodId"] == "dc.source-potential"]
    surface_reference = [item for item in boundary_conditions if item["methodId"] == "dc.reference-potential"]
    electrode_source = [
    ...
    if surface_mode:
        ...
        source_rule, reference_rule = surface_source[0], surface_reference[0]
        source = surface(scene, target_group(source_rule, "surface"))
        reference = surface(scene, target_group(reference_rule, "surface"))
